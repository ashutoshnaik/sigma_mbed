...
// MG MAXREFDES73#
extern uint8_t packet_0_buf[20], packet_1_buf[20], packet_2_buf[20], sweeping, sweepingDone;
// MG MAXREFDES73#
...
void HrpsProcMsg(wsfMsgHdr_t *pMsg)
{
  ...
  {
	// MG:
	if(sweeping && sweepingDone)
	{
		/* send notification */
		AttsHandleValueNtf((dmConnId_t) pMsg->param, HRS_HRM_HDL, 20, packet_1_buf);

		SysTick_Wait(0x147); //10 ms

		AttsHandleValueNtf((dmConnId_t) pMsg->param, HRS_HRM_HDL, 20, packet_2_buf);
		sweeping = 0;
		sweepingDone = 0;
	}
	else
		AttsHandleValueNtf((dmConnId_t) pMsg->param, HRS_HRM_HDL, 20, packet_0_buf);
